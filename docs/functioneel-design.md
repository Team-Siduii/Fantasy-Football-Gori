...
- Elke draft-pick synchroniseert direct naar het persoonlijke `manager-state` record van de gekoppelde manager-email. Bij login/herladen leest `/api/manager/state` altijd de ingelogde manager-email en repareert een ontbrekende/verouderde My Team-state vanuit het draft-teamroster, zodat browser sluiten/openen of opnieuw inloggen hetzelfde eigen team toont. Ook bestaande ronde-snapshots worden bij draft-sync overschreven met de actuele draftselectie, zodat de Team-pagina met `roundNumber` direct dezelfde picks toont.
...
FR-085: Bij elke My Team-read controleert `/api/manager/state` de ingelogde manager-email en herstelt het persoonlijke team vanuit het draft-teamroster als de manager-state ontbreekt of verouderd is; draftpagina toont daarnaast per team een live roster-overzicht op basis van team-roster-state.
