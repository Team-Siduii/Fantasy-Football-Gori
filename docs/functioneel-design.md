...
- Voor WK `Mijn team` default de geselecteerde speelronde naar de **actieve** ronde zodra die bezig is; alleen buiten een actieve ronde valt de view terug op de laatst gespeelde ronde. De eerste client-side hydrate en de daaropvolgende round-snapshot requests gebruiken dezelfde voorkeursronde, zodat `Mijn team` niet op een oudere knockout-ronde blijft hangen terwijl de nieuwe ronde al live is.
...
- Persistente JSON-writes naar `gori_fantasy_state` gebruiken één getypte `INSERT ... ON CONFLICT DO UPDATE` write-path, zodat production writes geen ongetypeerde/ongebruikte prepared-statement parameters meer naar Postgres sturen. Hierdoor blijven manager-state- en transfer-round writes ook op Neon/Vercel Postgres stabiel werken.
...
- WK round-matching voor teamweergaves gebruikt bij een geselecteerde ronde alleen spelers die in die exacte round-snapshot/evenementset voorkomen voor `roundPoints` en per-ronde advancement. Historische `totalPoints` blijven zichtbaar, maar oude roundpunten van eerder uitgeschakelde landen mogen niet doorlekken naar latere rondes.
- In WK `Mijn team` blijft round-persistence tijdens rondenavigatie vergrendeld totdat de nieuw gekozen ronde volledig is gehydrateerd. Een eerder zichtbare opstelling of formatie uit ronde N mag daardoor nooit meer als write-back in ronde N+1 terechtkomen alleen omdat de gebruiker naar een andere ronde bladert of de client nog late responses verwerkt.
- WK beschikbaarheid voor toekomstige knockout-rondes mag niet terugvallen op historische spelerpunten zodra de nieuwe ronde zelf nog geen eigen snapshot heeft. De actieve landen voor zo'n ronde worden afgeleid uit het officiële bracket + afgeronde feeder-duels (bijv. winnaars/verliezers van eerdere `duel`-referenties), zodat spelers van uitgeschakelde landen vóór de aftrap van de volgende ronde al als inactief verschijnen.
- In `Bekijk team` voor WK wordt bij een geselecteerde ronde de getoonde `teamCurrentRoundPoints` en `teamTotalPoints` server-side opnieuw opgebouwd uit de actuele round-/total-point maps van de opgeslagen lineup + bank, in plaats van blind te vertrouwen op een eerder persistente round-score snapshot. Daardoor blijven advancement-bonussen en live score-correcties direct zichtbaar, ook als een oudere persisted team-score nog niet is herberekend.
...
- Transfers voor poulewedstrijden verlopen in 4 expliciete fases per ronde:
  - Fase 1 · verkopen verzamelen: elke manager kiest eerst maximaal 1 reguliere handmatige verkoop via dropdown + knop `Verkopen toevoegen`; die keuze verschijnt in een verzamelblok `spelers te verkopen` en kan daar met `Undo` worden teruggedraaid zolang de manager nog niet op `Verkoop afronden` heeft gedrukt. Zodra `Verkoop afronden` is geklikt, staat die verkoop voor die manager vast en worden de verkoopcontrols voor die manager vergrendeld totdat alle managers fase 1 hebben afgerond. Uitgeschakelde/inactieve spelers uit de eigen selectie mogen in dezelfde fase extra aan die verkooplijst worden toegevoegd zonder die ene reguliere verkoop te verbruiken. Tijdens deze fase toont de manager-UI expliciet welke managers hun verkopen nog niet hebben afgerond, zodat zichtbaar is op wie de groep nog wacht
...
- In WK `Mijn team` gebruikt ook de **huidige** geselecteerde ronde altijd de round-snapshot van die ronde als bron van waarheid; de pagina mag voor de actieve ronde dus niet terugvallen op de top-level/live `manager-state`, zodat `Mijn team` en `Bekijk team` voor dezelfde ronde exact dezelfde selectie tonen.
- In WK `Mijn team` is een `pendingSellId` alleen transfer-UI-status en geen directe teammutatie: een speler die in verkoopwachtrij staat blijft daarom zichtbaar in de opgeslagen round-snapshot en op het veld/de bank, maar krijgt in de eigen teamweergave alleen een visuele verkoopmarkering. `Mijn team` en `Bekijk team` mogen zo'n speler dus niet lokaal wegfilteren of vervangen door een open slot zolang de transfer-round resolutie nog niet definitief is verwerkt.
- De verkooprij in `Mijn team` resolveert gequeue’de spelerkaarten niet alleen uit de actuele zichtbare squad-snapshot maar ook uit de geladen spelers-/marktdata, zodat een tweede verkoop (bijv. een uitgeschakelde/inactieve WK-speler) zichtbaar in de rij blijft staan. Na elke toevoeging reset de sell-selector terug naar de placeholder, zodat opeenvolgende verkopen op mobiel en native selects betrouwbaar opnieuw gekozen kunnen worden.
- Voor live incidentherstel is er een admin-only cleanup-pad om exact één foutieve of legacy nested `managerStates` key uit de gedeelde persistente WK/Eredivisie `manager-state` te verwijderen, zonder andere managerrecords te herschrijven. Dit wordt alleen gebruikt bij duplicate/split-brain manager-identiteiten nadat de canonieke teamstate al readback-gevalideerd is.
...
- WK advancement points zijn een aparte puntsoort naast match-events: de badge toont per geselecteerde ronde alleen de advancement van die ronde (bijv. `⚡+5`), terwijl het cumulatieve spelerstotaal deze advancement wel meeneemt over alle eerdere rondes heen. De getoonde team-rondepunten (`teamCurrentRoundPoints`) en persistente WK round-score snapshots tellen die advancement voor basis en bank wél mee, zodat badge, ranglijst en teamtotaal semantisch dezelfde rondescore representeren. Advancement-badges zijn zichtbaar vanaf knockoutronde 3 en blijven dus ook zichtbaar in latere rondes zoals 4, 5 en 6 zolang die ronde advancement bevat. Ook read-API's die een specifieke ronde teruggeven (zoals `/api/wk/players?round=N`) moeten in `advancementPoints` alleen die **ronde-delta** tonen en nooit het cumulatieve advancement-totaal van eerdere rondes. Advancement wordt bepaald op team/land-progressie per ronde; als opgeslagen `MW`-eventrows geen `team_name` bevatten, moet de scorer het land read-time herleiden via player-history metadata zodat ook bankspelers of niet-ingevallen spelers van een doorgaan land alsnog hun advancement krijgen. Bij knockoutrondes waarin een land doorgaat zonder bruikbaar `MW`-event (bijv. na strafschoppen of 0-0/door op een andere beslissingsregel), mag de scorer fallbacken op deelname van dat land in de **volgende** ronde zolang expliciete `MW`-eventrows voor de huidige ronde ontbreken én die volgende ronde een echte winnaar-doorstroom representeert (dus minder wedstrijden dan de huidige ronde). Plaatsingswedstrijden zoals een troostfinale/3e-plaats-duel mogen nooit als advancement-bron tellen voor de vorige ronde. Zodra expliciete `MW`-events voor de huidige ronde wél bestaan, blijven die leidend en mag een toekomstige of stale bracketmatch geen afvaller alsnog een advancement-badge geven.
...
- Uitgeschakelde/inactieve spelers blijven zichtbaar in de transfermarkt, maar worden uitgegrijsd en als `Uitgeschakeld`/niet-koopbaar gemarkeerd in plaats van stil uit de lijst te verdwijnen.
...
- Persistente Gori write-paths casten storage-identiteit (`state_key`, `store_name`, `scope`, `manager_key`) expliciet naar `text` in de Postgres update/insert-query. Daardoor blijven `manager-state`, `auth-state` en transfergerelateerde writes werken op production poolers/runtimes die ongetypeerde prepared-statement parameters anders afwijzen met `could not determine data type of parameter $2`.
...
- FR-085b: In WK `Mijn team` mag rondenavigatie nooit een oude clientstate terugschrijven naar de nieuw gekozen ronde. Persistence van formatie, lineup en bank is daarom geblokkeerd zolang de nieuwe ronde nog hydrateert of zolang de zichtbare state-provenance niet aantoonbaar bij de geselecteerde ronde hoort.
...
- 2026-07-09: Persistente JSON-writes casten nu `state_key`, `store_name`, `scope` en `manager_key` expliciet naar `text` in de update/insert-query, zodat production writes naar `manager-state` en `auth-state` niet meer vastlopen op Postgres/Neon poolers met `could not determine data type of parameter $2`.
...
- 2026-07-12: Admin manual-team-repair schrijft transfer-round correct terug op de actuele arrayvelden (`buyPlayerIds`/`resolvedTransfers`) in plaats van verouderde singular velden, zodat production builds/deploys niet meer stuklopen op typefouten en live WK repairs beschikbaar blijven.
...
- 2026-07-12: WK rondepunten gehard tegen semantische drift tussen match-events en advancement-badges: `teamCurrentRoundPoints` in `Bekijk team` en persistente round-score snapshots tellen advancement nu expliciet mee (met halve bankpunten), zodat teamtotaal, ranking en badge-breakdown dezelfde rondescore weergeven.
- 2026-07-13: `Bekijk team` berekent WK-rondepunten voor wissels nu op de gecombineerde rondescore (`roundPoints + advancementPoints`) vóór halvering/afronding. Daardoor tellen advancement-badges in ronde 6 en latere knockoutrondes voor alle managers consistent mee in `teamCurrentRoundPoints`, ook wanneer een bankspeler zowel matchpunten als advancement heeft.
