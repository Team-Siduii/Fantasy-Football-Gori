...
- Persistente WK self-heal (`repairManagerTeamFromDraftArtifactsPersistent`) gebruikt dezelfde positie-gestuurde auto-formation als de frontend/My Team-opbouw: bij een repair vanuit `team-roster` of draft-picks wordt de ruwe `manager-state` dus opnieuw opgebouwd tot een geldige 11+4 verdeling op basis van spelerposities, in plaats van simpelweg de eerste 11 ids als basis te bewaren.
- `/api/manager/view-team` draait vóór het uitlezen altijd dezelfde draft-artifact repair-flow als `/api/manager/state`, zodat gedeelde teamweergaves na een deploy/herlaadmoment niet kunnen achterblijven op een oude pick-order slicing terwijl My Team zelf al een positie-correct team toont.
...
- Spelers die in een volgende WK-ronde niet meer in de actuele availability/scoring-snapshot voorkomen, blijven zichtbaar in opgeslagen teamweergaves maar krijgen de status `inactive`: ze worden uitgegrijsd in `Mijn team` en `Bekijk team`, en zijn in de transfermarkt niet meer koopbaar. Zo blijft expliciet zichtbaar dat het land is uitgeschakeld of de speler niet meer actief is.
...
- 2026-06-15: WK `manager/state` self-healt nu ook corrupte round snapshots vóór uitlezen; als top-level team al compleet is maar een ronde-snapshot nog op een oude 1-spelerkeeper-state staat, wordt die snapshot eerst teruggetrokken naar de canonieke 15-speler bron zodat `Mijn team` niet meer kan terugvallen op ghost state.
...
- 2026-06-19: Team-hydration voor `Mijn team` behoudt nu de opgeslagen lineup- en bankvolgorde uit `manager-state` als bron van waarheid. De UI probeert saved WK-squads niet meer opnieuw in een rigide `GK/DEF/MID/FWD` bankverdeling te persen; alleen echt ontbrekende of inactieve spelers worden vervangen door open/inactive slots.
- 2026-06-19: WK ronde-navigatie in `Mijn team` gebruikt latest-only request guards voor spelers, matchdata en teamsnapshot-hydration. Daardoor kan terugnavigeren naar een nieuwere ronde niet meer per ongeluk de punten van een oudere, later teruggekomen response laten staan.
