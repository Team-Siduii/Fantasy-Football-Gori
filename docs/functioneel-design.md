...
- Elke draft-pick synchroniseert direct naar het persoonlijke `manager-state` record onder de canonieke `managerId` van de gekoppelde manager; e-mail en profielvelden blijven alleen alias-/contactdata. Bij login/herladen leest `/api/manager/state` altijd de ingelogde managersessie, resolveert die eerst naar dezelfde canonieke `managerId` en repareert een ontbrekende/verouderde My Team-state eerst vanuit het draft-teamroster en, als dat ontbreekt of out-of-sync is, vanuit de persistente draft-picks. Die repair gebruikt manager-aliasen uit auth-account, participant-config (`managerId`, label, email) en profieldata (naam/teamnaam/email), zodat ook typo's of oude sessie-emails niet meer leiden tot een leeg teamoverzicht. Legacy email-keyed manager-state records worden bij de eerstvolgende save automatisch naar `managerId` gemigreerd. Ook bestaande ronde-snapshots worden bij draft-sync overschreven met de actuele draftselectie, zodat de Team-pagina met `roundNumber` direct dezelfde picks toont.
...
FR-084: Elke draft `pick`/`return` synchroniseert direct naar persistente team-roster-state per team én naar het persoonlijke My Team record onder de canonieke `managerId` van de manager die via league-config/profiel/aliasen aan het draftteam gekoppeld is; bestaande ronde-snapshots van die manager worden eveneens naar de actuele draftselectie gezet, zodat teamselecties niet alleen in draft-log maar ook direct in accountgebonden teamstate en round-scoped Team-weergave landen.
FR-085: Bij elke My Team-read controleert `/api/manager/state` de ingelogde managersessie, resolveert die eerst naar de canonieke `managerId` en herstelt het persoonlijke team eerst vanuit het draft-teamroster; als dat roster ontbreekt of sessie-email/participant-email niet exact matchen, gebruikt de repairflow ook draft-picks plus manager-aliasen uit auth-account, participant-config en profieldata zodat hetzelfde draftteam zichtbaar blijft na typo-fixes, accountdrift of verloren roster-state. Legacy email-keyed manager-state records blijven leesbaar en migreren bij de eerstvolgende write automatisch naar `managerId`. Draftpagina toont daarnaast per team een live roster-overzicht op basis van team-roster-state.
...
- 2026-06-14: Productie krijgt nu een expliciete Vercel cron voor `GET /api/wk/sync-points` elke 15 minuten; de endpoint blijft buiten actieve/recent afgelopen wedstrijden vanzelf `skipped` teruggeven zodat de scheduler goedkoop en veilig kan blijven poll-en.
