...
- 2026-07-09: De admin-noodroute `/api/admin/force-update-state` cast nu ook zijn `state_key`/`scope` queryparameters expliciet naar `text`, zodat handmatige live teamreparaties op production niet vastlopen op dezelfde Postgres/Neon prepared-statement typingfout als de reguliere write-path.
